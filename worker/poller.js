...
export async function pollAll() {
  ...
  console.log(`[AUSPEX worker] senses: usgs ${usgs.length}, gdacs ${gdacs.length}, eonet ${eonet.length}, space ${space.length}, breakthroughs ${brk.length}`);
  const events = await runReasoningPass([...usgs, ...gdacs, ...eonet, ...space, ...brk], process.env.AUSPEX_LLM_KEY ?? null);
